...
int OpBasic :: min_diameter_tree_value(Graph& g) {
	int min_diameter = INT_MAX, n = g.getQtdVertices(), ex = INT_MAX;
	...
	for(int v : g.adjList(u)) {
		if(v == p) continue;
			...
			if(low[v] >= in[u]) {
				...
				while(!stV.empty() && low[stV.back()] >= in[u]) {
					int w = stV.back();
					for(auto [x, y] : edBlock[w]) {
						if(visV[x] != vID) {
							visV[x] = vID;
							V.push_back(x);
						}
			...
			edBlock[u].emplace_back(u, v);
		}
	}

	stV.emplace_back(u);

	if(p != -1) edBlock[u].emplace_back(p, u);
